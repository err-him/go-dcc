 ...
 *
 * Rhyolite Software DCC 1.3.163-1.61 $Revision$
 */
...
/* ensure that the client context socket is open so that we can borrow it */
static u_char
ck_clnt_soc(DCC_EMSG *emsg, DCC_CLNT_CTXT *ctxt)
{
...
create_helper_soc(HELPER_GEN *gen, DCC_CLNT_CTXT *ctxt, void *logp)
{
	...
	if (!ck_clnt_soc(&emsg, ctxt)) {
		thr_error_msg(logp, "DNSBL %s", emsg.c);
		new_gen();
		return 0;
	}
	dcc_mk_loop_su(&gen->su, ctxt->soc[0].loc.sa.sa_family, 0);
	   ...
	   const char *id)
{
	 ...
	 * been handled the parent thread is gone. */
	   ...
	   int resp_len)
{
	...
	if (!ck_clnt_soc(&emsg, dlw->dcc_ctxt)) {
		thr_error_msg(dlw->log_ctxt, "%s DNSBL %s", req->id, emsg.c);
		return 0;
	}
	...
	if (DCC_IS_TIME(now.tv_sec, gen->idle_restart,
			HELPER_IDLE_RESTART)
	    && gen->idle_helpers > 0) {
		if (HELPERS_DEBUG())
	...
	if (gen->idle_helpers <= 0) {
		...
		} else if (!new_helper(gen, dlw->dcc_ctxt,
				       dlw->log_ctxt, req->id)) {
	...
	if (!DCC_SU_EQ(&dlw->dcc_ctxt->soc[0].rem, &gen->su)
	    && !helper_clnt_soc_connect(&emsg, dlw->dcc_ctxt, &gen->su)) {
		thr_error_msg(dlw->log_ctxt,
	...
	/* Send the request. */
	i = send(dlw->dcc_ctxt->soc[0].s, req, req_len, 0);
	if (i != req_len) {
		if (i < 0) {
			serrno = errno;
			thr_error_msg(dlw->log_ctxt,
	...
	for (;;) {
		...
		if (us < 0)
			us = 0;
		pollfd.fd = dlw->dcc_ctxt->soc[0].s;
		...
		i = recvfrom(dlw->dcc_ctxt->soc[0].s, resp, resp_len,
			     0, &recv_su.sa, &su_len);
		if (i < 0) {
			if (DCC_BLOCK_ERROR())
				continue;
